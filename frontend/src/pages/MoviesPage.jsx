          ...
          <div className="animate-fade-in" style={{ width: '100%', maxWidth: '440px', background: 'var(--bg-modal)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-xl)', padding: '40px', position: 'relative', boxShadow: 'var(--shadow-lg)' }}>
            ...
            <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
              ...
              <div style={{ width: '100%', display: 'flex', gap: '8px' }}>
                <input readOnly value={groupSessionData.joinUrl} style={{ flex: 1, height: '42px', padding: '0 12px', background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)', fontSize: '12px', color: 'var(--text-secondary)', fontFamily: 'var(--font-body)', outline: 'none' }} />
                <Button variant="primary" size="sm" iconLeft={<Copy size={13} />} onClick={() => navigator.clipboard.writeText(groupSessionData.joinUrl)}>Copy</Button>
              </div>
              ...
              <Button variant="primary" size="lg" fullWidth iconLeft={<Ticket size={15} />} onClick={() => { setShowShareModal(false); navigate(`/seats?showtimeId=${groupSessionData.joinUrl.split('showtimeId=')[1].split('&')[0]}&session=${groupSessionData.sessionCode}`) }}>
                Enter Seating Map
              </Button>
            </div>
          </div>
